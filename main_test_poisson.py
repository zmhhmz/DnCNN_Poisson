...
def parse_args():
    ...
    parser.add_argument('--sigma', default=0.15, type=float, help='noise level')
    parser.add_argument('--model_dir', default=os.path.join('models', 'DnCNN_sigma0.15_15epo'), help='directory of the model')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                y = np.random.poisson(255.0*x*args.sigma)/(255.0*args.sigma)#x + np.random.normal(0, args.sigma/255.0, x.shape)  # Add Gaussian noise without clipping
